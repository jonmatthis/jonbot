...
def main(directory):
    ...
    for result in cc_results:
        if result.complexity > CC_THRESHOLD:
            print(
                f"{result.nick_name} in {result.filename}, Line {result.lineno}, CC: {result.complexity}"
            )
    ...
    for filename, result in mi_results.items():
        ...
        print(
            f"  Blank lines: {result.blank}, Single-line comments: {result.single_comments}"
        )
    print("\n" + "-" * 50)  # Add separator for readability
    ...
    for filename, result in halstead_results.items():
        ...
        print(
            f"  Volume: {round(result.volume, 2)}, Difficulty: {round(result.difficulty, 2)}"
        )
        print(
            f"  Effort: {round(result.effort, 2)}, Time: {round(result.time, 2)}, Bugs: {round(result.bugs, 4)}"
        )
