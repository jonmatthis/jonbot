...
- Your software is a multi-layered architecture with multiple frontends (Layer 0: Frontends - e.g. Discord, Telegram, Webapp, etc) each connected to a common set of FastAPI controller (Layer1: API Interface -  e.g. `/chat`, `/voice_to_text`, etc),  which routes requests and returns responses from the core_processing processes (Layer 2, e.g. housing thigns like `ai/` and `audio_transcription/` submodules). There is also a data layer that interacts with a MongoDB database
