...
class ContextMemoryHandler(BaseModel):
    ...
    async def context_memory_document(self) -> ContextMemoryDocument:
        if self.current_context_memory_document is None:
            logger.trace(f"Current context memory document is None, loading from database...")
    ...
    async def _upsert_request(self) -> ContextMemoryDocumentRequest:
        ...
        return ContextMemoryDocumentRequest.build_upsert_request(document=document,
                                                                 database_name=self.database_name)

    async def _upsert_context_memory(self):
        logger.debug(f"Upserting context memory for context route: {self.context_route.dict()}")
        try:
            await self.database_operations.upsert_context_memory(await self._upsert_request)
        except Exception as e:
            logger.exception(e)
            raise

    async def update(self, message_buffer: list, summary: str, token_count: int):
        logger.debug(f"Updating context memory for context route: {self.context_route.dict()} - summary: {summary}")
        document = await self.context_memory_document
        document.update(message_buffer=message_buffer,
                        summary=summary,
