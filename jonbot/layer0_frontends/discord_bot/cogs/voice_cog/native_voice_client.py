...
import discord
from discord import VoiceProtocol, Client, VoiceChannel
from discord.backoff import ExponentialBackoff

from jonbot.layer0_frontends.discord_bot.cogs.voice_cog import ffi
...
class NativeVoiceClient(VoiceProtocol):
    ...
    async def on_voice_state_update(self, data: dict) -> None:
        ...
        logger.info('Voice Session ID: %s', session_id)
    ...
    async def on_voice_server_update(self, data: dict) -> None:
        if self._voice_server_received.is_set():
            logger.info('Ignore extraneous voice server update')
            return
        server_id = data['guild_id']
        token: Optional[str] = data.get('token')
        endpoint: Optional[str] = data.get('endpoint')
        if endpoint is None or token is None:
            logger.warning('Awaiting endpoint... This requires waiting.')
            return
        logger.info('Voice Gateway Endpoint: %s', endpoint)
        # [host, ':', port]
    ...
    async def reconnect_handler(self, reconnect, timeout):
        ...
        while True:
            try:
                ...
                logger.info('Voice connection got a clean close %s', e)
