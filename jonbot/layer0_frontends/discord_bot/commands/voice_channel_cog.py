...
class VoiceChannelCog(discord.Cog):
    ...
    async def join(self, ctx: discord.ApplicationContext):
        ...
        self.voice_connections[ctx.guild.id] = ctx.guild.voice_client
    ...
    async def start_recording(self, ctx: discord.ApplicationContext, sink: Sinks, duration: int = 10):
        ...
        voice_connection = self.voice_connections.get(ctx.guild.id, None)
    ...
    async def stop(self,
        ...
        if ctx.guild.id in self.voice_connections:
            voice_connection = self.voice_connections[ctx.guild.id]
            voice_connection.stop_recording()
            del self.voice_connections[ctx.guild.id]
            if self.status_message:
                await self.status_message[ctx.guild_id].delete()
                del self.status_message[ctx.guild_id]
    ...
    async def finished_callback(self,
        ...
        await self.status_message[channel.guild.id].edit(
            content=f"{VOICE_RECORDING_PREFIX} {' '.join(recorded_users)}.", files=files
            )
